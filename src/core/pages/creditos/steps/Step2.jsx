...
export function Step2() {
    ...
    return (
        <div className="container-step2 w-full">
            ...
            <div className="flex flex-column md:flex-row gap-5 w-full justify-content-between">
                <div className="step2-productos-search flex flex-column mt-2 w-20rem">
                    {products.map((product, index) => (<ProductCardOrden key={index} product={product} addProductoToOrden={addProductoToOrden}></ProductCardOrden>))}
                </div>
                <div className="cart-products flex flex-column w-25rem">
                    ...
                    <div className="body-cart">
                        {
                            orden?.items?.map((item,index)=>(<OrderItem item={item} key={index} deleteItem={removeProductoFromOrden}></OrderItem>))
                        }
                    </div>
                    <div className="footer-cart" style={{fontWeight:"bold"}}><strong className="mr-3">Total: </strong> S/ {parseFloat(orden?.total).toFixed(2)}</div>
                </div>
